...
add_pdn_stripe -grid stdcell_grid -layer {metal1} -width {0.17} -pitch {2.4} -offset {0} -followpins

# 3. Define Macro Grid (Power Rings/Straps)
# Uses Metal4 (vertical) and Metal5 (horizontal) for power mesh
add_pdn_stripe -grid stdcell_grid -layer {metal4} -width {0.48} -pitch {50.0} -offset {2.0} -extend_to_core_ring
add_pdn_stripe -grid stdcell_grid -layer {metal5} -width {0.48} -pitch {50.0} -offset {2.0} -extend_to_core_ring
