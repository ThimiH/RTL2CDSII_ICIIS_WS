...
We will install OpenSTA by building it from source, as demonstrated in NPTEL Lecture 31.
...
You will see a detailed text report in the terminal.

\subsection{1. Setup Check (Max Delay)}
Look for the \textbf{Slack (VIOLATED)} or \textbf{Slack (MET)} line.
\begin{itemize}
...
If Slack is positive, the design meets the setup timing.

\subsection{2. Hold Check (Min Delay)}
Look for the hold check report.
\begin{itemize}
    \item \textbf{Slack:} (Data Arrival) - (Data Required).
\end{itemize}

If Slack is positive, the design meets hold timing.