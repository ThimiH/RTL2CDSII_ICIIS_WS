...
\section{Overview}
In this chapter, we perform logic synthesis: the process of converting our High-Level RTL (Verilog) into a gate-level netlist mapped to a specific Standard Cell Library. We will use the open-source tool \textbf{Yosys}.

\section{Installation}
We will install the Yosys Open Synthesis Suite on Ubuntu.

\subsection{1. Install Dependencies}
First, install the required prerequisites for building Yosys:
...
\end{lstlisting}
Alternatively, you can launch the Yosys interactive shell and source the script manually:
\begin{lstlisting}[language=bash]
yosys
...
\begin{itemize}
    \item \textbf{Mapped Gates:} You should no longer see equations like \texttt{assign F = A + B}. Instead, you will see instantiated gates from the Sky130 HD library such as \texttt{sky130\_fd\_sc\_hd\_\_nand2\_1}, \texttt{sky130\_fd\_sc\_hd\_\_mux2\_1}, and \texttt{sky130\_fd\_sc\_hd\_\_dfrtp\_1}.
...
\begin{itemize}
    ...
    \item \textbf{Timing Constraints:} At this stage, we have not yet applied strict timing constraints (SDC). The netlist is optimized primarily for area. Timing optimization will be handled in the next chapter (STA).
\end{itemize}